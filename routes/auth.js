...
router.post('/verify-code', async (req, res) => {
  ...
  try {
    ...
    if (!user.verificationCode || user.verificationCode !== code || user.codeExpires < Date.now()) {
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign(
      { user: { id: user._id.toString(), name: user.name, email: user.email, university: user.university, department: user.department, isAdmin: user.isAdmin } },
      JWT_SECRET,
      { expiresIn: '1d' }
    );
